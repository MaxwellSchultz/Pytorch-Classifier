...
class NeuralNet(nn.Module):
    def __init__(self, input_size, hidden_size, num_classes, num_hidden_layers):
        ...
        for layer in range(num_hidden_layers):
            self.hidden_ls.append(nn.Linear(hidden_size,hidden_size))
...
def model_pipeline(hyperparams):
    ...
    with wandb.init(project="multi-perceptron-model-leaky", config=hyperparams):
        config = wandb.config
...
def make(config):
    ...
    # get model 
    model = NeuralNet(D,config.hidden_size,C,config.num_hidden_layers).to(device)
...
def get_data(features_file, labels_file, N, D, slice=5):
